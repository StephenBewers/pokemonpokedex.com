        ...
        >
          ...
          <div className="modal-info-panel" ref={this.infoPanelRef}>
            ...
            <ModalRow id="modal-top-row">
              <PokemonTypes pokemon={pokemon} key={`types-${pokemon.variant.id}`} filterBtnClick={filterBtnClick} />
              ...
              <PokemonWeight pokemon={pokemon} key={`weight-${pokemon.variant.id}`} />
              <PokemonGender pokemon={pokemon} key={`gender-${pokemon.species.id}`} />
            </ModalRow>
            <ModalRow id="modal-centre-section">
              <ModalColumn>
                <PokemonBaseStats pokemon={pokemon} key={`base-stats-${pokemon.variant.id}`} />
                <PokemonAbilities pokemon={pokemon} key={`abilities-${pokemon.variant.id}`} />
                <PokemonTraining pokemon={pokemon} key={`training-${pokemon.variant.id}`} />
              </ModalColumn>
              <ModalColumn>
                <PokemonTypeEffectiveness pokemon={pokemon} key={`type-effectiveness-${pokemon.variant.id}`} filterBtnClick={filterBtnClick} />
              </ModalColumn>
            </ModalRow>
            <PokemonEvolution pokemon={pokemon} clickHandler={this.refreshModal} key={`evolution-${pokemon.variant.id}`} />
            <PokemonOtherForms pokemon={pokemon} formsOfCurrentVariantReceived={formsOfCurrentVariantReceived} refreshModal={this.refreshModal} key={`other-forms-${pokemon.variant.id}`} />
          </div>
        </section>
      </div>
    );
